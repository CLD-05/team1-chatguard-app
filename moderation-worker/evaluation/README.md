...
| 강한 긍정 표현 | 40 | 욕설처럼 보이는 강한 표현이지만 실제로는 칭찬/감탄인 문장의 오탐 확인 |
...
.\run-model.ps1 -ModelVersion "unsmile-weighted-v1" -BlurThreshold 0.40 -CleanPenalty 0.10
```
...
cd C:\CE\ChatGuard\team1-chatguard-app\moderation-worker
.\.venv\Scripts\Activate.ps1
python evaluation\collect_moderation_results.py --model-version unsmile-weighted-v1 --run-name unsmile-weighted-v1
```
...
python evaluation\collect_moderation_results.py --model-version unsmile-weighted-v1 --prefix "[T1] " --run-name unsmile-weighted-v1-t1
```
