...
.\run-model.ps1 -ModelVersion "unsmile-v1" -BlurThreshold 0.40 -CleanPenalty 0.05
```
...
- 기본값: `BLUR_THRESHOLD=0.40`, `CLEAN_PENALTY=0.05`

threshold나 clean penalty를 바꿔 실험할 때는 worker를 종료한 뒤 아래처럼 다시 실행한다.

```powershell
cd C:\CE\ChatGuard\team1-chatguard-app\moderation-worker
.\run-model.ps1 -ModelVersion "unsmile-weighted-t035-c005" -BlurThreshold 0.35 -CleanPenalty 0.05
```
...
```powershell
python evaluation\collect_moderation_results.py --model-version unsmile-weighted-t035-c005 --run-name unsmile-weighted-t035-c005
```
