...
export default function LogsTab({ guard }) {
  ...
  useEffect(() => {
    guard(getLogs({ stage: stage || undefined, verdict: stage === 'AI' ? 'BLOCK' : undefined, page }))
      .then((data) => { setLogs(data.content ?? []); setTotalPages(data.total_pages ?? 0) })
      .catch(() => {})
      ...
      {loading ? <Spinner /> : (
        <>
          ...
          <div className="flex justify-center gap-2 mt-4">
            ...
            <span className="text-xs text-gray-500 self-center">{page + 1} / {totalPages || 1}</span>
