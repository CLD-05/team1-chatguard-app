...
export default function DashboardTab({ guard }) {
  ...
  const STAT_CARDS = [
    { label: '총 메시지',        key: 'total_messages',  sub: (s) => `오늘 +${s.today_messages?.toLocaleString() ?? 0}`, color: 'text-emerald-400' },
    { label: '키워드 차단',      key: 'keyword_blocked', sub: (s) => ((s.keyword_blocked / Math.max(s.total_messages, 1)) * 100).toFixed(1) + '%', color: 'text-red-400' },
    { label: 'AI 블러',          key: 'ai_blurred',      sub: (s) => ((s.ai_blurred / Math.max(s.total_messages, 1)) * 100).toFixed(1) + '%', color: 'text-amber-400' },
    { label: '활성 채팅방',      key: 'active_rooms',    sub: null, color: 'text-gray-500' },
  ]

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STAT_CARDS.map(({ label, key, sub, color }) => (
          <div key={key} className="bg-gray-800/60 border border-gray-700/50 rounded-xl p-5">
            <p className="text-xs text-gray-500 mb-2">{label}</p>
            <p className="text-3xl font-semibold text-white tracking-tight">
              {stats ? stats[key]?.toLocaleString() : <span className="text-gray-600">—</span>}
            </p>
            {sub && stats && (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-700/60">
                ...
                <th className="px-4 py-3 text-left text-xs text-gray-600 font-medium">유저</th>
            ...
            <tbody className="divide-y divide-gray-700/40">
              {recentLogs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-700/20 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-xs text-gray-400 whitespace-nowrap">{log.sender_name ?? '—'}</td>
              ...
              {recentLogs.length === 0 && (
                <tr><td colSpan={4} className="py-10 text-center text-gray-600 text-sm">검열 내역이 없습니다.</td></tr>
              )}
            </tbody>
          </table>
        </div>
